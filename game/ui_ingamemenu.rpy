init 2 python:
    ...
    def custom_menu(items, is_narrator, window_style='menu_window', interact=True, with_none=None, **kwargs):
        ...
        if interact:
            ...
            return rv
        return None
    def show_menu(menuitems, style='menu', caption_style='menu_caption', choice_style='menu_choice', choice_chosen_style='menu_choice_chosen', choice_button_style='menu_choice_button', choice_chosen_button_style='menu_choice_chosen_button', location=None, focus=None, default=False, active=None, **properties):
    ...
    def ingamebutton(text, clicked, previously=None):
        ...
        if previously:
            bgim = im.Composite(None, (0, 0), 'ui/bg-choice.png', (577, 2), im.Alpha('ui/bt-cf-checked.png', 0.5))
        else:
            if (previously) is not (None):
                bgim = im.Composite(None, (0, 0), 'ui/bg-choice.png', (577, 2), im.Alpha('ui/bt-cf-unchecked.png', 0.5))
            else:
                bgim = 'ui/bg-choice.png'
        bg = Fixed(bgim, xmaximum=608)
        this_idle = LiveComposite((608, 35), (0, 0), ingm_bg(bg), (0, 0), ingm_btn(img))
        this_hover = LiveComposite((608, 35), (0, 0), ingm_bg(bg), (0, 0), ingm_btn_hover(img))
    ...
    def ingamebutton_dead(text, is_active=False):
        ...
        if is_active:
            ...
            bg = Fixed(im.MatrixColor('ui/bg-choice.png', ((im.matrix.desaturate()) * (im.matrix.brightness(-(0.20000000000000001)))) * (im.matrix.opacity(0.29999999999999999))), xmaximum=608)
        ui.fixed(xpos=0.5, ymaximum=35, xmaximum=800)
